...
export function ConfigPage({ mode }: { mode: RuntimeMode }) {
  ...
  const save = async () => {
    ...
    if (firstSetup && password !== passwordConfirmation) return setMessage('保存失败：两次输入的主密码不一致。');
    try {
      ...
      setApiKey(''); setPassword(''); setPasswordConfirmation(''); setMessage('已保存 DeepSeek API Key。下一步：测试连接。'); await refresh();
  ...
  const unlock = async () => {
    ...
    if (response.ok) { setPassword(''); setPasswordConfirmation(''); await refresh(); }
  };
  ...
  return <div style={{ maxWidth: 600, margin: '0 auto' }}>
    ...
    {firstSetup && <div className="card"><h3>第 2 步：保存 API Key</h3><p>请访问 <a href="https://platform.deepseek.com/api-keys" target="_blank" rel="noreferrer">DeepSeek 控制台</a> 创建 Key。</p><label htmlFor="deepseek-api-key">DeepSeek API Key（不会回显）</label><input id="deepseek-api-key" className="input" type="password" value={apiKey} onChange={event => setApiKey(event.target.value)} /><label htmlFor="master-password">首次设置主密码（12–128 个字符）</label><input id="master-password" className="input" type="password" value={password} onChange={event => setPassword(event.target.value)} /><label htmlFor="master-password-confirmation">确认主密码</label><input id="master-password-confirmation" className="input" type="password" value={passwordConfirmation} onChange={event => setPasswordConfirmation(event.target.value)} /><button className="btn btn-primary" onClick={() => void save()}>保存 DeepSeek API Key</button></div>}
