...
export function ConfigPage({ mode }: { mode: RuntimeMode }) {
  ...
  if (mode === 'public') return <div className="card"><h2>本地配置</h2><p>公共演示模式不会接收或保�?API Key�?/p></div>;
  ...
  const save = async () => {
    ...
    if (firstSetup && !validPassword(password)) return setMessage('保存失败：主密码需�?12�?28 个字符�?);
    try {
      ...
      setKey(''); setPassword(''); setMessage('已保�?DeepSeek API Key。下一步：测试连接�?); await refresh();
    } catch { setMessage('保存失败：无法连接本地服务�?); }
  };
  const unlock = async () => {
    if (!password) return setMessage('解锁失败：请输入主密码�?);
    try { const response = await fetch('/api/config/unlock', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ masterPassword: password }) }); if (!response.ok) return setMessage('解锁失败：主密码不正确�?); setPassword(''); setMessage('凭据已解锁。下一步：输入或更�?API Key�?); await refresh(); } catch { setMessage('解锁失败：无法连接本地服务�?); }
  };
  const testConnection = async () => {
    ...
    catch { setMessage('连接测试失败：无法连�?DeepSeek，请检查网络或 API Key�?); }
  ...
  return <div style={{ maxWidth: 600, margin: '0 auto' }}><h2 className="section-title">DeepSeek 配置</h2>
    <div className="card"><h3>�?1 步：准备本地凭据</h3><p>主密码只用于加密本地 API Key，不会上传或回显�?/p>{status.state === 'locked' ? <><label htmlFor="master-password">输入主密码以解锁</label><input id="master-password" className="input" type="password" value={password} onChange={e => setPassword(e.target.value)} /><button className="btn btn-primary" onClick={() => void unlock()}>解锁凭据</button></> : <p>凭据状态：{status.state === 'empty' ? '尚未设置' : '已解�?}</p>}</div>
    {status.state !== 'locked' && <div className="card"><h3>�?2 步：保存 DeepSeek API Key</h3><p>请访�?<a href="https://platform.deepseek.com/api-keys" target="_blank" rel="noreferrer">DeepSeek 控制�?/a> 创建 Key�?/p><label htmlFor="deepseek-api-key">DeepSeek API Key（不会回显）</label><input id="deepseek-api-key" className="input" type="password" value={key} onChange={e => setKey(e.target.value)} />{firstSetup && <><label htmlFor="master-password">首次设置主密码（12�?28 个字符）</label><input id="master-password" className="input" type="password" value={password} onChange={e => setPassword(e.target.value)} /></>}<button className="btn btn-primary" onClick={() => void save()}>保存 DeepSeek Key</button></div>}
    {status.hasKey && <div className="card"><h3>�?3 步：测试连接</h3><p>确认 API Key 可以正常访问 DeepSeek�?/p><button className="btn btn-secondary" disabled={testing} onClick={() => void testConnection()}>{testing ? '测试中�? : '测试连接'}</button><button className="btn btn-danger" onClick={() => void clear()}>删除配置</button></div>}
